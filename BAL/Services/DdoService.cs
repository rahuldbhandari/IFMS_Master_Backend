...
namespace IFMS_Master_Backend.BAL.Services
{
    public class DdoService : IDdoService
    {
        ...
        public async Task<IEnumerable<DdoModel>> getAllDetail()
        {
            List<Ddo> allDdoData = (List<Ddo>)await _DdoRepo.GetAllAsync();
        ...
        {
            Ddo ddo = await _DdoRepo.GetSingleAysnc(ddo => ddo.Id == Id && ddo.IsDelete == false);
